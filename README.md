...
### Auto-detection of type by string representation.
...
* `1.01` can be recognised either as a decimal number or as a date (1<sup>st</sup> of January)
...
* Is there any guarantee that all vital things that are based on Excel computations are safe and don't contain similar bugs? No, it isn't.
* The tool stops being just a tool. It makes specialists correct the activity area to make the tool possible to be used. And this thing is not connected with something fundamental for this area (representation models). It is connected with chosen names and their wrong recognitions with this tool. This is not what is expected from the convenient programming tool.

### Everything is a `String` or an instance of other built-in types.
...
* They are not extendable because they only work in this case: if a user wants some more complex condition (for example, `num -> num < 3 || num > 5`), he/she has to use intermediate cells.
...
<table>
  ...
  <tr>
    ...
    <td align='center' valign='top'>
      ...
      <p align='justify'>
        C++ is a very difficult (for non-programmers) language as it is a system programming language.
      </p>
    </td>
    <td valign='top'>
...
```
    ...
    <td align='center' valign='top'>
      ...
      <p align='justify'>
        In spite of a powerful type system, it is easy to get a memory leak in C++. Having undefined behaviour is also a big disadventage. You cannot guarantee that some code would behave the same on different platforms. That does not suit <b>Excel</b>-like sheets.
      </p>
    </td>
    <td align='center' valign='top'>
  ...
  <tr>
    ...
    <td align='center' valign='top'>
      ...
      <ul align='justify'>
        ...
        <li>Good point is that Python makes a user follow the correct indentation. However, it is possible to notify about bad indents in other languages. It is not built there, but the our context is the app, not the pure language.</li>
      </ul>
    </td>
    <td align='center' valign='top'>
  ...
  <tr>
    ...
    <td align='center' valign='top'>
      ...
      <p align='justify'>
        Java memory management is based on GC. All classes instances are references so are easy to be effectively taken as a function argument (without the necessity to think about reference type, moving, copying as in C++). That makes programming a lot easier for newbies like users of the app. Java uses OOP as the main paradigm. However, it may be a bit difficult for those who have no programming skills to deal with it.
      </p>
    </td>
    <td align='center' valign='top'>
      <pre lang='diff'>- 3 points</pre>
...
* This `BigInteger` oughts to support negative numbers as they can be used as an operand to calculate some other addresses.
* Operators should also support indexing by `Int`, `Long` instances because they are easier to write.
* `java.math.BigInteger` type shouldn't be used as the `BigInteger` itself because used implementation may change. `java.math.BigInteger`:
  * Has slow multiplication ([Karatsuba algorithm](https://en.wikipedia.org/wiki/Karatsuba_algorithm))
  * Is JVM only
  
  So its wrapper should be used instead now.

### Infinity
* There are `Infinity` objects: `PositiveInfinity`, `NegativeInfinity`, `Infnity`.
...
* There should be a `typealias Inf = Infinity` alias.

### Extended `BigInteger`
* It is just a `BigInteger` that also supports signed `Infinity`s.
* It may be helpful in functions like `longSize` property for progressions.

### Ranges
* As all numeric types, these `BigInteger`s have corresponding `Progression` and `Range` classes.
* However, I expect these classes to be a bit wider than there `stdlib` analogues, they should:
  1. Support infinity ranges/progressions so the actual (since now) `BigInteger` is an extended one.
  2. Consequently, be `Sequence` but not `Iterable` by default (be lazy).
  3. Have effective `size`, `contains`, `intersect`, `union` and other operations that have generalized implementation for `Iterable`s and `Sequence`s. They must have the same semantics when possible.
  4. There should be some functions that have analogues in the previous point, but have a wider return type. The example is the `longSize` property that uses extended `BigInteger` as the return type and doesn't reduce the size to the `Int` range bounds.
  5. *This should be discussed as may seem useless and alien to `stdlib` ones: constant infinite progressions (where `step` is 0).*
* `Range` instances, but not `Progression` ones need to be returned from the functions when `step` is 1, so `Progression` constructor should be internal or even private, static abstract method `fromClosedRange` and `progression`/`range` functions should be used instead of it from externals of the `Progression` implementation.

### Regions
* Indexation is only via numbers, not letters because it is easier to do so from code and it unifies column and row naming.
...
* However, there should be a `RegionSet` class that
  ...
  * If some region is already contained by some of the regions, it is not added. Example: There are two rectangles (vertical and horizontal lines inside)
    ```
    ▤▤
    ▤▤▥
    ▤▤▥	
...
* Regions can be converted to `Sequence` or `Iterable` instances using row-by-row and column-by-column order.
* Regions have `width`, `height` and `cellCount` properties.
* Regions must contain such (extension) methods of `Iterable`s and `Sequence`s that apply to them too.
...
* You can specify how to render your data by deriving the `Renderable` interface. By default, the rendered content is just a rendered `toString`. Good examples are diagrams and mathematical equations. *Both are expected to be builtins.*
* `Iterable`s, `Sequence`s should have a suitable `Renderable` instance.
...
* All regions know what code depends on the values in them. If type changes, all dependent scripts are recompiled and rerun, if only content changes (for non-code regions), they are just rerun.
...
* Lots of features such as Diagrams and Equations must have also a wizard that makes the same thing as code generating, but with better (**Excel**-like) UI (but worse automatisation). It is more convenient for users.

### Code support
* Evaluation is lazy because the number of cells to evaluate may be infinite.
* Receiver is a `Region` (its most narrow ancestor) for the code in it.
...
* There are 2 approaches:
  1. Code is compiled into `.jar` files. Each region with code has its `.jar`. It is necessary to re-download the classes with the same classloader because otherwise the same classes would be recognised as the different ones. Proxies should be used to determine who depends on who in runtime. So if you use some property of a region, you use a proxy instance that would memoize the dependency existence. Scripts are recompiled sequentially to be able to find the loops.
  2. Use incremental compilation. It is more convenient but has a problem to be solved that all not compiled code needs to continue working during the recompilation of other parts.
* Libraries can be downloaded from scripts in cells or settings of the app.
* There should be preloaded libraries that contain different economical and some other helpful functions.
...
* It should be possible to specify a policy for some projects only.
* It should be possible to add a certificate verifying the authors and to choose the policy depending on the author.
